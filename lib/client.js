window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// dsh 的外部 store 方法可能通过 `this` 读取内部状态。React 会脱离对象调用
		// subscribe/getSnapshot；统一经这一层调用，兼容新旧内核并保持订阅函数稳定。
		...
		const css = [
			...
			".dstFooterBtnLabelWrap{position:relative;display:inline-flex;align-items:center;min-width:0}",
			...
			//
			// **别删这条，也别指望别的插件已经写了**：Git / 市场 / 余额各自也带一份
			// 完全相同的声明。以前只有这里写，结果「装了终端面板的机器一切正常、只装
			// 市场 + 余额的机器上图标挤成一行」—— 一个插件的样式在替别的插件兜底，
			// 而任何一个插件都可能被单独安装。四份重复是有意的，不是漏删。
			"[class*=\"footerActions\"]{flex-direction:column;align-items:stretch}",
			// 小徽标：面板关闭期间有后台命令结束时显示 6px 圆点。
			// 失败默认红点，成功用绿点；打开面板即清除。
			// 宽按钮时放到「终端」文字右上角并闪烁；折叠按钮仍放在按钮右上角。
			".dstBadge{position:absolute;top:4px;right:4px;width:6px;height:6px;border-radius:50%;background:var(--dsw-alias-state-error-primary,#f0617a);pointer-events:none;animation:dstBadgeBlink 1.2s ease-in-out infinite}",
			".dstBadgeSuccess{background:var(--dsw-alias-state-success-primary,#3fb950)}",
			".dstFooterBtnLabelWrap .dstBadge{top:-2px;right:-6px}",
			// —— 面板 ——
			...
			// 背板用 specific-sidebar-fill（深色 #1b1b1c），与左侧菜单同底。别用 bg-overlay：
			// 它在深色下是 #61666b 的中灰，浮在 #151517 的界面上就是一块灰板子。
			".dstPanel{position:fixed;right:20px;bottom:20px;z-index:20;width:min(760px,calc(100vw - 40px));height:min(520px,70vh);display:flex;flex-direction:column;background:var(--dsw-specific-sidebar-fill,#1b1b1c);border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.08));border-radius:12px;box-shadow:0 16px 48px rgba(0,0,0,.32);color:var(--dsw-alias-label-primary,#f9fafb);font-size:13px;overflow:hidden;pointer-events:none;opacity:0;transform:translateY(8px);transition:opacity .16s ease,transform .16s ease}",
			...
			".dstTab{flex:none;box-sizing:border-box;display:inline-flex;align-items:center;gap:6px;max-width:220px;height:24px;padding:0 8px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));border-radius:6px;background:var(--dsw-alias-bg-layer-1,#232324);color:var(--dsw-alias-label-secondary,#cfd3d6);font-size:12px;font-family:inherit;cursor:pointer;transition:background .15s ease,color .15s ease,border-color .15s ease}",
			".dstTab:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.08));border-color:var(--dsw-alias-border-l3,rgba(255,255,255,.2))}",
			".dstTabActive{background:var(--dsw-specific-input-major,#2c2c2e);border-color:var(--dsw-alias-state-business-primary,#4d6bfe);color:var(--dsw-alias-label-primary,#f9fafb)}",
			...
			// 只有识别为 error / warning 的行才加左竖条和颜色；普通 stderr/stdout 行保持
			// 默认白色，不能因为走了 stderr 就整行标红（Vite/Webpack 常把普通日志也写 stderr）。
			...
			".dstLineWarn{border-left:2px solid var(--dsw-alias-state-warn-primary,#e3a008);padding-left:6px;margin-left:-2px;color:var(--dsw-alias-state-warn-primary,#e3a008)}",
			...
			".dstStopBtn{flex:none;display:inline-flex;align-items:center;justify-content:center;height:20px;padding:0 8px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));border-radius:5px;background:var(--dsw-alias-button-ghost-active-fill,#2a2a2e);color:var(--dsw-alias-label-secondary,#cfd3d6);font-size:11px;font-family:inherit;cursor:pointer;transition:background .15s ease,color .15s ease,border-color .15s ease}",
			...
			"@keyframes dstBadgeBlink{0%,100%{opacity:1}50%{opacity:.2}}",
		...
		const tagId = "dsh-terminal-panel/panel.css";
		if (typeof document !== "undefined") {
			// 不能只在标签不存在时才注入：dsh 的 client-hmr 重载插件时会重新执行 factory，
			// 但不会清掉之前注入的 style 标签。若沿用「已存在就跳过」的写法，改 CSS 后
			// 页面上留着的还是旧样式（比如新增的 dstLineWarn 不生效）。因此每次都要把
			// 最新 CSS 写回已有的 style 标签；标签不存在时才新建。
			const existing = document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]");
			if (existing !== null) {
				existing.textContent = css;
			} else {
				const tag = document.createElement("style");
				tag.dataset.plugin = "dsh-terminal-panel";
		...
		// 徽标：面板关闭期间有后台命令结束时置位，打开面板即清除。
		// 记录 { kind, id }：kind 区分绿点/红点，id 用于打开面板时定位到触发徽标的标签。
		// 若先成功后又失败，错误优先；同状态多次结束时，定位到最近结束的那个。
		function createBadgeStore() {
			...
			return {
				...
				set(kind, id) {
					const next = badge && badge.kind === "error" && kind !== "error" ? badge : { kind, id };
					if (!badge || badge.kind !== next.kind || badge.id !== next.id) {
						badge = next;
						notify();
					}
				},
				clear: () => { if (badge !== null) { badge = null; notify(); } }
			};
		}
		 ...
		 * 判断一行输出是否为真正的 error。
		 *
		 * 不再用「stderr = error」的简化：stderr 里大量普通日志（Building for production…、
		 * More info: …）也需要保持默认白色。只有带明确 error/fatal/failed/exception/ERR!
		 * 等错误标记的行才标红。
		 */
		function isErrorLine(text) {
			return /\b(?:error|fatal|failed|exception)\b|(?:^|[A-Za-z])Error\b|^\s*npm ERR!|\bERR!\b/i.test(text);
		}

		/**
		 * 判断一行输出是否为 warning/deprecation。
		 *
		 * 只负责 warning，不再隐含 error 判断；渲染时先看 error，再看 warning，
		 * 两者都不是就保持默认颜色。
		 */
		function isWarningLine(text) {
			// npm warn / WARNING: / warning: / warn: / deprecation / DeprecationWarning 等。
			return /(^|[\s(:[])(?:(?:npm|pnpm|yarn)\s+)?(?:warn(?:ing)?|deprecat(?:ed|ion))\b|warning\s*:|(?:^|[A-Za-z])Warning\b/i.test(text);
		}
		...
		function connectStream(view) {
			...
			const url = `/api/dsdesktop/terminal/stream?id=${encodeURIComponent(view.id)}&token=${encodeURIComponent(view.token)}&since=${view.lastSeq}`;
			let es;
			try {
		...
		function TerminalFooterAction({ wide, t, store, badgeStore }) {
			...
			const badge = useStore(badgeStore);
			...
			return react_jsx_runtime.jsxs("button", {
				...
				children: [
					...
					wide ? react_jsx_runtime.jsxs("span", {
						className: "dstFooterBtnLabelWrap",
						children: [
							react_jsx_runtime.jsx("span", { className: "dstFooterBtnLabel", children: t("terminal.panel.title") }),
							badge ? react_jsx_runtime.jsx("span", { className: "dstBadge" + (badge.kind === "success" ? " dstBadgeSuccess" : "") }) : null
						]
					}) : null,
					!wide && badge ? react_jsx_runtime.jsx("span", { className: "dstBadge" + (badge.kind === "success" ? " dstBadgeSuccess" : "") }) : null
				]
			});
		}
		//#endregion
		...
		function TerminalPanel({ t, store, badgeStore, themeStore, workspacesList, sessionsList }) {
			...
			const scheme = useStore(themeStore);
		...
		function TerminalPanelBody({ t, open, dark, store, badgeStore, workspacesList, sessionsList }) {
			const wsState = useStore(workspacesList);
			const sessState = useStore(sessionsList);
			...
			// 徽标轮询的锚点：只认「面板关闭之后」新发生的结束。
			const lastExitAtRef = react.useRef(0);
			// 触发呼吸灯的那个会话 id：打开面板时优先定位到它，并跳过自动新建标签。
			const pendingBadgeIdRef = react.useRef(null);
			...
			react.useEffect(() => {
				if (!open) {
					...
					// 徽标轮询：面板关闭期间检测新结束的命令，成功/失败都提示。
					lastExitAtRef.current = Date.now();
					const timer = setInterval(async () => {
						try {
							const res = await getJson("/api/dsdesktop/terminal/sessions");
							if (!res.ok || !res.data) return;
							for (const s of res.data.sessions) {
								if (s.lastExit && s.lastExit.at > lastExitAtRef.current) {
									badgeStore.set(s.lastExit.code === 0 ? "success" : "error", s.id);
				...
				// 先把呼吸灯对应的会话 id 记下来，再清徽标；后续用它定位标签。
				pendingBadgeIdRef.current = badgeStore.getSnapshot()?.id ?? null;
				badgeStore.clear();
				let alive = true;
				getJson("/api/dsdesktop/terminal/sessions").then((res) => {
					if (!alive) return;
					if (!res.ok) {
						pendingBadgeIdRef.current = null;
					...
					setViews((prev) => {
						...
						for (const s of list) {
							...
							if (found) {
								found.workspaceId = s.workspaceId || found.workspaceId;
								found.cwd = s.cwd;
			...
			const createTab = react.useCallback(async () => {
				...
				try {
					const res = await postJson("/api/dsdesktop/terminal/session", { workspaceId: linkedWorkspaceId });
					if (!res.ok) {
			...
			react.useEffect(() => {
				...
				// 有呼吸灯：说明关闭期间有任务结束，打开面板应该定位到那个已有标签，
				// 而不是自动新建一个当前工作区标签。
				if (pendingBadgeIdRef.current) {
			...
			// 定位到触发呼吸灯的那个标签。放在 auto-create 之后：让 auto-create 先看到
			// pendingBadgeIdRef 并跳过新建，再由这里把 activeId 指过去。
			react.useEffect(() => {
				if (!open || !pendingBadgeIdRef.current) return;
				if (views.some((v) => v.id === pendingBadgeIdRef.current)) {
					setActiveId(pendingBadgeIdRef.current);
					pendingBadgeIdRef.current = null;
				} else if (sessionsLoaded) {
					// 会话已经不存在（比如期间被关闭）时，别一直拿着旧 id。
					pendingBadgeIdRef.current = null;
				}
			}, [open, sessionsLoaded, views]);
			...
			const closeTab = react.useCallback(async (id) => {
				...
				try {
					await postJson("/api/dsdesktop/terminal/close", { id: view.id, token: view.token });
				} catch { /* 忽略 */ }
			...
			const exec = react.useCallback(async (raw) => {
				...
				try {
					const res = await postJson("/api/dsdesktop/terminal/exec", { id: view.id, token: view.token, command });
					if (!res.ok) {
			...
			const sendSignal = react.useCallback(async () => {
				...
				try {
					await postJson("/api/dsdesktop/terminal/signal", { id: view.id, token: view.token });
				} catch { /* 忽略 */ }
			}, [activeView]);
			...
			const openExternal = react.useCallback(async () => {
				...
				if (!view) return;
				try {
					const res = await postJson("/api/dsdesktop/terminal/open-external", { id: view.id, token: view.token });
			...
			const clearScreen = react.useCallback(async (view) => {
				if (!view) return;
				try {
					await postJson("/api/dsdesktop/terminal/clear", { id: view.id, token: view.token });
				} catch { /* 忽略 */ }
				view.lines = [];
			...
			const complete = react.useCallback(async (ta) => {
				...
				let res;
				try {
					res = await postJson("/api/dsdesktop/terminal/complete", {
						id: view.id,
						token: view.token,
						input: ta.value,
					...
					react_jsx_runtime.jsxs("div", {
						...
						children: [
							...
							renderLines.map((line, i) => {
								...
								const text = line.segments.map((seg) => seg.t).join("");
								const isError = isErrorLine(text);
								const isWarning = isWarningLine(text);
								let cls = "dstLine";
								if (line.stream === "err" || line.stream === "out") {
									// 普通输出行即使来自 stderr 也保持默认颜色；只有明确带
									// error/warning 标记才染色。error 优先于 warning。
									if (isError) cls += " dstLineErr";
									else if (isWarning) cls += " dstLineWarn";
