window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 用 "success" / "error" 区分绿点和红点；若先成功后又失败，错误优先。
		function createBadgeStore() {
			...
			return {
				...
				set(kind) {
					const next = kind === "error" || badge === "error" ? "error" : kind === "success" ? "success" : badge;
					if (badge !== next) { badge = next; notify(); }
				},
				...
				children: [
					...
					wide ? react_jsx_runtime.jsxs("span", {
						...
						children: [
							...
							badge ? react_jsx_runtime.jsx("span", { className: "dstBadge" + (badge === "success" ? " dstBadgeSuccess" : "") }) : null
						]
					}) : null,
					!wide && badge ? react_jsx_runtime.jsx("span", { className: "dstBadge" + (badge === "success" ? " dstBadgeSuccess" : "") }) : null
				]
			});
		}
		//#endregion
						...
						try {
							...
							for (const s of res.data.sessions) {
								if (s.lastExit && s.lastExit.at > lastExitAtRef.current) {
									badgeStore.set(s.lastExit.code === 0 ? "success" : "error");
