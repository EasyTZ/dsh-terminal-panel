...
// 服务名只有坏处 —— ctx.provide 撞名是直接抛异常的，等于在 boot 阶段杀掉内核、
// 桌面端黑屏，和 loader 的 duplicate loader entry id 是同一类事故。同理路由统一
// 挂在 /api/dsdesktop/ 前缀下：webServer 的 register 对重复 (kind, path) 也是直接抛，
// 而内核会自己热更新，上游哪天加一条 /api/terminal/* 我们就撞上了。
//
...
//
// 信任假设要讲明白：token **不是权限隔离**。GET /api/dsdesktop/terminal/sessions 把每个
...
const ROUTE_PREFIX = "/api/dsdesktop/terminal";
...
export const inject = ["webServer", "workspaceRegistry", "shell"];
  ...
  for (const [suffix, handler] of ROUTES) {
    ...
    ctx.effect(() => ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        // apply 执行时缓存会拿到 undefined。listen 未完成时宽放（同源页面此刻
        // 也还在启动阶段，发不出请求）。
